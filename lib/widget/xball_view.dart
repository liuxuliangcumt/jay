...
class XBallView extends StatefulWidget {
  ...
  ///需要展示的关键词
  List<Song> keywords = List();

  ///需要高亮的关键词
  List<Song> highlight = List();
  List<Point> points = [];

  XBallView({Key key, @required this.mediaQueryData, BuildContext context}) : super(key: key) ;
...
class _XBallViewState extends State<XBallView> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    //动画
    controller = AnimationController(duration: Duration(milliseconds: 40000), vsync: this);
...
/*  @override
  void didUpdateWidget(XBallView oldWidget) {
    ...
    //数据有变化，重新初始化点
    if (oldWidget.keywords.length != widget.keywords.length) {
      generatePoints(widget.keywords);
    }
  }*/
